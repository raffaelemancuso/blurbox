...
from video_cover_region import Area, VideoInfo
...
def encoders(monkeypatch):
    ...
    def use(names=ALL_ENCODERS):
        monkeypatch.setattr(vcr, "available_encoders", lambda: frozenset(names))
    use()
    return use
...
def test_parse_time(text, secs):
    assert vcr.parse_time(text) == pytest.approx(secs)
...
def test_fmt_time(secs, text):
    assert vcr.fmt_time(secs) == text


@pytest.mark.parametrize("secs", [0, 1 / 30, 12.345, 3599.999, 7322.5])
def test_fmt_parse_roundtrip(secs):
    assert vcr.parse_time(vcr.fmt_time(secs)) == pytest.approx(secs, abs=5e-4)
...
def test_effect_chain_black_uses_range_appropriate_black():
    limited = vcr.effect_chain("black", 100, 100, 20, full_range=False)
    full = vcr.effect_chain("black", 100, 100, 20, full_range=True)
...
def test_effect_chain_blur_and_pixelate():
    assert vcr.effect_chain("blur", 100, 100, 7, False) == [("gblur", "sigma=7")]
    assert vcr.effect_chain("pixelate", 300, 120, 16, False) == [
        ("scale", "18:7:flags=area"), ("scale", "300:120:flags=neighbor")]
    # a block bigger than the area still leaves one block, never zero
    assert vcr.effect_chain("pixelate", 10, 10, 50, False)[0] == ("scale", "1:1:flags=area")


def test_build_filter_single_area_whole_video():
    fg = vcr.build_filter([(Area(mode="blur", strength=9), (10, 20, 30, 40))], info())
...
def test_build_filter_ranges_with_offset_and_half_frame_pad():
    ...
    fg = vcr.build_filter([(a, (0, 0, 10, 10))], info(offset=0.5, fps=25.0))
...
def test_build_filter_chains_areas_in_order():
    ...
    fg = vcr.build_filter(items, info())
    parts = fg.split(";")
...
def test_build_filter_keeps_source_pixel_format_or_none():
    fg10 = vcr.build_filter([(Area(), (0, 0, 10, 10))], info(pix_fmt="yuv420p10le"))
    assert fg10.endswith("format=yuv420p10le[v]")
    assert vcr.build_filter([(Area(), (0, 0, 10, 10))], info(pix_fmt="")).endswith("null[v]")


def test_build_filter_full_range_black():
    fg = vcr.build_filter([(Area(mode="black"), (0, 0, 10, 10))], info(pix_fmt="yuvj420p"))
...
def test_video_encoder_follows_source(encoders, codec, ext, encoder):
    args, _ = vcr.plan_streams(info(codec=codec), Path("out" + ext), 20)
    assert value_after(args, "-c:v") == encoder
    assert value_after(args, "-crf") == "20"


def test_video_reencode_is_noted(encoders):
    _, notes = vcr.plan_streams(info(codec="prores"), Path("o.mov"), 18)
    assert any("re-encoded as H264" in n for n in notes)
    _, notes = vcr.plan_streams(info(codec="h264"), Path("o.mp4"), 18)
...
def test_hevc_falls_back_to_h264_without_x265(encoders):
    ...
    args, notes = vcr.plan_streams(info(codec="hevc"), Path("o.mp4"), 18)
...
def test_hevc_in_mp4_gets_apple_tag(encoders):
    mp4, _ = vcr.plan_streams(info(codec="hevc"), Path("o.mp4"), 18)
    mkv, _ = vcr.plan_streams(info(codec="hevc"), Path("o.mkv"), 18)
...
def test_colour_metadata_is_passed_on(encoders):
    ...
    args, _ = vcr.plan_streams(info(codec="hevc", color=color), Path("o.mp4"), 18)
...
def test_audio_copied_or_converted(encoders, audio, ext, expected):
    args, notes = vcr.plan_streams(info(audio=[audio]), Path("o" + ext), 18)
...
def test_every_audio_track_is_mapped(encoders):
    args, _ = vcr.plan_streams(info(audio=["aac", "pcm_s24le", "ac3"]), Path("o.mp4"), 18)
...
def test_subtitles_kept(encoders, sub, ext, codec):
    args, _ = vcr.plan_streams(info(subtitles=[sub]), Path("o" + ext), 18)
...
def test_picture_subtitles_left_out_of_mp4_with_note(encoders):
    args, notes = vcr.plan_streams(info(subtitles=["hdmv_pgs_subtitle", "subrip"]),
                                   Path("o.mp4"), 18)
...
def test_attachments_only_kept_in_mkv(encoders):
    mkv, mkv_notes = vcr.plan_streams(info(attachments=2), Path("o.mkv"), 18)
    mp4, mp4_notes = vcr.plan_streams(info(attachments=2), Path("o.mp4"), 18)
...
def test_data_streams_noted(encoders):
    _, notes = vcr.plan_streams(info(data_streams=1), Path("o.mkv"), 18)
...
def test_faststart_only_for_mp4_and_mov(encoders):
    for ext, has in [(".mp4", True), (".mov", True), (".m4v", True), (".mkv", False)]:
        args, _ = vcr.plan_streams(info(), Path("o" + ext), 18)
        assert ("+faststart" in args) is has


def test_render_cmd_shape():
    cmd = vcr.build_render_cmd(Path("in.mp4"), Path("out.mp4"), "[0:v]null[v]", ["-c:v", "libx264"])
...
"""
    class Result:
        stdout = listing
    monkeypatch.setattr(vcr.subprocess, "run", lambda *a, **k: Result())
    vcr.available_encoders.cache_clear()
    try:
        assert vcr.available_encoders() == {"libx264", "aac", "mov_text"}
    finally:
        vcr.available_encoders.cache_clear()
